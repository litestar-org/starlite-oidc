...
from starlette.status import HTTP_307_TEMPORARY_REDIRECT, HTTP_500_INTERNAL_SERVER_ERROR
...
class TestOIDCAuthentication:
    ...
    REDIRECT_URI: str = f"{BASE_URL}/redirect"
    ...
    def app(self, auth: OIDCAuthentication, session_config: SessionCookieConfig) -> Starlite:
        @get(path=self.POST_LOGOUT_REDIRECT_PATH, name=self.POST_LOGOUT_VIEW, before_request=auth.oidc_logout)
        def logout() -> str:
            return "logged out"
    ...
    def init_app(self, request, app: Starlite, auth: OIDCAuthentication) -> None:
        param = getattr(request, "param", False)
        logout_views = self.POST_LOGOUT_VIEW if param is False else param
        init_app_args = {"app": app, "redirect_uri": self.REDIRECT_URI, "logout_views": logout_views}
        auth.init_app(**init_app_args)

    @pytest.fixture()
    def request_factory(self, request, app: Starlite) -> Request:
    ...
    def request_client(self, app: Starlite, session_config: SessionCookieConfig) -> TestClient:
        with TestClient(app=app, base_url=self.BASE_URL, session_config=session_config) as client:
            yield client
    ...
    @pytest.fixture()
    def introspection_result(self, request) -> Dict[str, Union[bool, List[str]]]:
        ...
        return {"active": active, "aud": audience, "scope": ' '.join(scope), "exp": int(time.time()) + 60}
    ...
    def test_init_app(self, auth: OIDCAuthentication, app: Starlite) -> None:
        assert auth._redirect_uri.geturl() == self.REDIRECT_URI
        assert self.PROVIDER_NAME in auth.clients
        assert auth._post_logout_redirect_paths in ([], [self.POST_LOGOUT_REDIRECT_PATH])

    def test_should_authenticate_if_no_session(self, auth: OIDCAuthentication, request_factory: Request) -> None:
        auth_redirect = auth.oidc_auth(scope=request_factory.scope, provider_name=self.PROVIDER_NAME)

        assert isinstance(auth_redirect, RedirectResponse) is True
        assert "auth_request" in request_factory.session
        assert request_factory.session[self.CURRENT_PROVIDER] == self.PROVIDER_NAME
    ...
    def test_should_not_authenticate_if_session_exists(
        ...
        UserSession(request_factory.session, self.PROVIDER_NAME).update(userinfo=userinfo)
        auth_status = auth.oidc_auth(scope=request_factory.scope, provider_name=self.PROVIDER_NAME)
        assert auth_status is None
        assert request_factory.user == userinfo
        assert self.PROVIDER_NAME in request_factory.session[self.CURRENT_PROVIDER]

    def test_reauthenticate_silently_if_session_expired(
        self, auth: OIDCAuthentication, request_factory: Request
    ) -> None:
        auth.clients[self.PROVIDER_NAME]._provider_configuration.session_refresh_interval_seconds = 60  # in seconds

        with mock.patch("time.time") as time_mock:
            time_mock.return_value = int(time.time()) - 1  # authenticated in the past
            UserSession(request_factory.session, self.PROVIDER_NAME).update()
        auth_redirect = auth.oidc_auth(scope=request_factory.scope, provider_name=self.PROVIDER_NAME)
    ...
    def test_expected_auth_response_mode_is_set(
        ...
        auth.clients[self.PROVIDER_NAME]._provider_configuration.auth_request_params = {"response_type": response_type}
        auth_redirect = auth.oidc_auth(scope=request_factory.scope, provider_name=self.PROVIDER_NAME)
        assert request_factory.session["fragment_encoded_response"] is expected
        assert isinstance(auth_redirect, RedirectResponse) is True

    @pytest.mark.parametrize("init_app, remove_uris", [(None, False), (POST_LOGOUT_VIEW, True), (None, True)],
                             indirect=["init_app"])
    @responses.activate
    def test_should_register_client_if_not_registered_before(
        ...
        client = auth.clients[self.DYNAMIC_CLIENT_PROVIDER_NAME]
        ...
        if remove_uris is True:
            ...
            expected_registration_request["redirect_uris"] = [self.REDIRECT_URI]
            ...
            if auth._post_logout_redirect_paths:
                expected_registration_request["post_logout_redirect_uris"] = [
                    f"{self.BASE_URL}{self.POST_LOGOUT_REDIRECT_PATH}"
                ]
            else:
        ...
        auth_redirect = auth.oidc_auth(scope=request_factory.scope, provider_name=self.DYNAMIC_CLIENT_PROVIDER_NAME)
    ...
    def test_handle_authentication_response(
        ...
        provider_config = auth.clients[self.PROVIDER_NAME]._provider_configuration

        with mock.patch("starlite_oidc.oidc.rndstr", side_effect=[self.STATE, "test-nonce"]):
            auth.oidc_auth(request_factory.scope, self.PROVIDER_NAME)
        ...
        request_client.get(url=f"{auth._redirect_uri.path}?state={self.STATE}&code=test-auth-code",
                           follow_redirects=True)

        session_storage = request_client.get_session_from_cookies()
        session = UserSession(session_storage, self.PROVIDER_NAME)
    ...
    def test_handle_implicit_authentication_response(
        ...
        auth.clients[self.PROVIDER_NAME]._provider_configuration.auth_request_params = {
        ...
        access_token_response["state"] = self.STATE
        ...
        if request_method == "GET":
            request_client.get(url=f"{auth._redirect_uri.path}?{access_token_response.to_urlencoded()}",
                               follow_redirects=True)
        else:
            request_client.post(url=str(auth._redirect_uri.path), data=access_token_response.to_urlencoded(),
                                headers=self.FORM_CONTENT_TYPE, follow_redirects=True)
    ...
    def test_handle_error_response(
        ...
        error_response = {"state": self.STATE, "error": "invalid_request", "error_description": "test error"}
        self.set_cookies(session=request_factory.session, test_client=request_client)
    ...
    def test_handle_error_response_no_stored_auth_request(
        ...
        missing_auth_request_error = {"error": "unsolicited_response",
    ...
    def test_oidc_logout_redirects_to_provider(
        self, request_factory: Request, id_token_store: IdTokenStore, request_client: TestClient,
            auth: OIDCAuthentication
    ) -> None:
        # Test with empty session. The logout route handler should run even if there is no session.
        assert request_client.get(url=self.POST_LOGOUT_REDIRECT_PATH).json() == self.LOGOUT_STATUS

        # Test with session.
        UserSession(request_factory.session, self.PROVIDER_NAME).update(id_token_jwt=id_token_store.id_token_jwt)
        self.set_cookies(session=request_factory.session, test_client=request_client)
        end_session_redirect_response = request_client.get(url=self.POST_LOGOUT_REDIRECT_PATH, follow_redirects=False)
        ...
        assert (
                ...
                == auth.clients[self.PROVIDER_NAME].provider_end_session_endpoint
        )
        assert query_params["id_token_hint"] == id_token_store.id_token_jwt
        assert query_params["post_logout_redirect_uri"] == f"{self.BASE_URL}{self.POST_LOGOUT_REDIRECT_PATH}"
        ...
        for _ in range(2):
            recall_response = request_client.get(url=self.POST_LOGOUT_REDIRECT_PATH,
                                                 params={"state": query_params["state"]})
            assert recall_response.json() == self.LOGOUT_STATUS
            assert request_client.get_session_from_cookies() == {}

    def test_oidc_logout_without_end_session_endpoint(self, auth: OIDCAuthentication,
                                                      request_factory: Request,
                                                      id_token_store: IdTokenStore,
                                                      request_client: TestClient) -> None:
        auth.clients.pop(self.DYNAMIC_CLIENT_PROVIDER_NAME)
        auth.clients[self.PROVIDER_NAME]._provider_configuration._provider_metadata.pop("end_session_endpoint")

        UserSession(request_factory.session, self.PROVIDER_NAME).update(id_token_jwt=id_token_store.id_token_jwt)
        request_client.get(url=self.POST_LOGOUT_REDIRECT_PATH)
        self.set_cookies(session=request_factory.session, test_client=request_client)
        end_session_redirect_response = request_client.get(url=self.POST_LOGOUT_REDIRECT_PATH)
        # Redirect to provider cannot occur if end-session endpoint URL is missing. The route handler will be called.
        assert end_session_redirect_response.json() == self.LOGOUT_STATUS

        # Ensure that user session has been cleared.
        session = request_client.get_session_from_cookies()
        assert all(provider_name not in session for provider_name in auth.clients)

    def test_using_unknown_provider_name_should_raise_exception(self, auth: OIDCAuthentication,
                                                                request_factory: Request) -> None:
        with pytest.raises(ValueError) as exc_info:
            auth.oidc_auth(request_factory.scope, provider_name="unknown")
        assert "unknown" in str(exc_info.value)

    def test_validate_access_token_should_not_refresh_access_token(self, auth: OIDCAuthentication,
                                                                   request_factory: Request,
                                                                   access_token_response: AccessTokenResponse) -> None:
        # Test with empty session
        assert auth.valid_access_token(request=request_factory) is None

        # Test with session
        # Don't add refresh token to prevent refresh.
        session = UserSession(request_factory.session, self.PROVIDER_NAME)
        session.update()
        assert auth.valid_access_token(request=request_factory) is None

        # Add access_token and expires_in to test the validity of the access token.
        access_token = self.ACCESS_TOKEN
        session.update(access_token=access_token, expires_in=access_token_response["expires_in"],
                       refresh_token=access_token_response["refresh_token"])
        # Access token is not expired yet so the access token will not be updated.
    ...
    @responses.activate
    def test_validate_access_token_should_refresh_access_token(self, expired: bool, forced: bool,
                                                               access_token_response: AccessTokenResponse,
                                                               auth: OIDCAuthentication, id_token_store: IdTokenStore,
                                                               request_factory: Request):
        ...
        responses.post(provider_config._provider_metadata["token_endpoint"], json=token_response)
        responses.get(provider_config._provider_metadata["jwks_uri"],
                      json={"keys": [id_token_store.id_token_signing_key.serialize()]})
        auth.valid_access_token(request=request_factory, force_refresh=forced)
        assert request_factory.session[self.PROVIDER_NAME]["access_token"] == self.ACCESS_TOKEN

    @responses.activate
    def test_should_return_none_if_token_refresh_request_fails(self, request_factory: Request,
                                                               access_token_response: AccessTokenResponse,
        ...
        responses.post(auth.clients[self.PROVIDER_NAME]._provider_configuration._provider_metadata[
            "token_endpoint"], json=token_response)
        assert auth.valid_access_token(request=request_factory) is None

    @pytest.mark.parametrize("request_factory, status", [(None, False),
                                                         (AUTHORIZATION_HEADER, True)],
                             indirect=["request_factory"])
    def test_should_check_for_authorization_header(self, auth: OIDCAuthentication, request_factory: Request,
                                                   status: bool) -> None:
        assert auth._check_authorization_header(request_factory.headers) is status
        if status is True:
            assert auth._parse_access_token(request_factory.headers) == self.ACCESS_TOKEN

    @pytest.mark.parametrize("introspection_result", [{"active": False}, {"aud": "no_client"}, {"scope": "extra"}, {}],
                             indirect=["introspection_result"])
    @responses.activate
    def test_introspect_token(self, introspection_result: Dict[str, Union[bool, List[str]]], request_factory: Request,
                              auth: OIDCAuthentication) -> None:
        request_factory._headers = self.AUTHORIZATION_HEADER
        request_factory.scope['route_handler'].opt['scopes'] = self.SCOPES

        client = auth.clients[self.PROVIDER_NAME]
        provider_config = client._provider_configuration
    ...
    @pytest.mark.parametrize("request_factory, auth_exception", [(None, NotAuthorizedException),
                                                                 (AUTHORIZATION_HEADER, PermissionDeniedException)],
                             indirect=["request_factory"])
    @responses.activate
    def test_token_auth_should_raise_exception_if_verification_fails(
            self, request_factory: Request, auth_exception: Union[NotAuthorizedException, PermissionDeniedException],
            auth: OIDCAuthentication, introspection_result: Dict[str, Union[bool, List[str]]]) -> None:
        introspection_result["active"] = False

        responses.post(auth.clients[self.PROVIDER_NAME]._provider_configuration._provider_metadata[
                           "introspection_endpoint"], json=introspection_result)
        with pytest.raises(auth_exception):
            auth.token_auth(request_factory.scope, self.PROVIDER_NAME)

    @pytest.mark.parametrize("request_factory", [AUTHORIZATION_HEADER], indirect=True)
    @responses.activate
    def test_token_auth_for_valid_access_token(self, auth: OIDCAuthentication,
        ...
        responses.post(auth.clients[self.PROVIDER_NAME]._provider_configuration._provider_metadata[
                           "introspection_endpoint"], json=introspection_result)
        assert auth.token_auth(request_factory.scope, self.PROVIDER_NAME) is None
        assert request_factory.auth == introspection_result

    @pytest.mark.parametrize("request_factory", [AUTHORIZATION_HEADER], indirect=True)
    @responses.activate
    def test_access_control_should_run_token_auth(self, auth: OIDCAuthentication,
                                                  introspection_result: Dict[str, Union[bool, List[str]]],
                                                  request_factory: Request) -> None:
        request_factory.scope['route_handler'].opt['scopes'] = self.SCOPES
        ...
        responses.post(auth.clients[self.PROVIDER_NAME]._provider_configuration._provider_metadata[
                           "introspection_endpoint"], json=introspection_result)
        assert auth.access_control(request_factory.scope, self.PROVIDER_NAME) is None
        assert request_factory.auth == introspection_result

    def test_access_control_should_fallback_to_oidc_auth(self, auth: OIDCAuthentication,
                                                         request_factory: Request) -> None:
        control = auth.access_control(request_factory.scope, self.PROVIDER_NAME)
        assert isinstance(control, RedirectResponse) is True

    @pytest.mark.parametrize("request_factory, introspection_result", [(AUTHORIZATION_HEADER, {"active": False})],
                             indirect=True)
    @responses.activate
    def test_access_control_should_deny_permission_if_verification_fails(
            self, auth: OIDCAuthentication, introspection_result: Dict[str, Union[bool, List[str]]],
            request_factory: Request) -> None:
        responses.post(auth.clients[self.PROVIDER_NAME]._provider_configuration._provider_metadata[
                           "introspection_endpoint"], json=introspection_result)
        with pytest.raises(PermissionDeniedException):
            auth.access_control(request_factory.scope, self.PROVIDER_NAME)
